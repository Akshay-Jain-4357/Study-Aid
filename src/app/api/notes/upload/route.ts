...
export async function POST(req: Request) {
  try {
    ...
    // Convert file to buffer for cloud storage upload
    ...
    const storageEndpoint = process.env.AWS_S3_ENDPOINT || '';

    let storageUrl = `https://placeholder-storage.com/${uniqueFileName}`;

    // Upload to cloud storage (GCS via S3-compatible API, or AWS S3)
    if (process.env.AWS_ACCESS_KEY_ID && process.env.AWS_SECRET_ACCESS_KEY) {
      ...
      // Build URL based on configured endpoint (GCS or AWS)
      if (storageEndpoint.includes('storage.googleapis.com')) {
        storageUrl = `https://storage.googleapis.com/${bucketName}/${uniqueFileName}`;
      } else {
        const region = process.env.AWS_REGION || 'us-east-1';
        storageUrl = `https://${bucketName}.s3.${region}.amazonaws.com/${uniqueFileName}`;
      }
    }
